...
RenderController::RenderController
    ...
    QObject::connect(   renderWindow->leedsGLRenderWidget,           SIGNAL(BeginScaledDrag(int, float, float)),
                        this,                                       SLOT(BeginScaledDrag(int, float, float)));
    QObject::connect(   renderWindow->leedsGLRenderWidget,           SIGNAL(ContinueScaledDrag(float, float)),
                        this,                                       SLOT(ContinueScaledDrag(float, float)));
    QObject::connect(   renderWindow->leedsGLRenderWidget,           SIGNAL(EndScaledDrag(float, float)),
...
void RenderController::zoomChanged(int value)
    ...
    renderParameters->zTranslate = (float) value / 100.0f;
...
void RenderController::emissiveLightChanged(int value)
    ...
    renderParameters->emissiveMaterial = (float) value / 100.0;
    
    // clamp it
    if (renderParameters->emissiveMaterial < LIGHTING_MIN)
        renderParameters->emissiveMaterial = LIGHTING_MIN;
    else if (renderParameters->emissiveMaterial > LIGHTING_MAX)
        renderParameters->emissiveMaterial = LIGHTING_MAX;
...
void RenderController::ambientLightChanged(int value)
    ...
    // reset the value (slider ticks are 1/100 each)
    renderParameters->ambientMaterial = (float) value / 100.0;
    
    // clamp it
    if (renderParameters->ambientMaterial < LIGHTING_MIN)
        renderParameters->ambientMaterial = LIGHTING_MIN;
    else if (renderParameters->ambientMaterial > LIGHTING_MAX)
        renderParameters->ambientMaterial = LIGHTING_MAX;
...
void RenderController::diffuseLightChanged(int value)
    ...
    // reset the value (slider ticks are 1/100 each)
    renderParameters->diffuseMaterial = (float) value / 100.0;
    
    // clamp it
    if (renderParameters->diffuseMaterial < LIGHTING_MIN)
        renderParameters->diffuseMaterial = LIGHTING_MIN;
    else if (renderParameters->diffuseMaterial > LIGHTING_MAX)
        renderParameters->diffuseMaterial = LIGHTING_MAX;
...
void RenderController::specularLightChanged(int value)
    ...
    // reset the value (slider ticks are 1/100 each)
    renderParameters->specularMaterial = (float) value / 100.0;
    
    // clamp it
    if (renderParameters->specularMaterial < LIGHTING_MIN)
        renderParameters->specularMaterial = LIGHTING_MIN;
    else if (renderParameters->specularMaterial > LIGHTING_MAX)
        renderParameters->specularMaterial = LIGHTING_MAX;
...
void RenderController::perspectiveCheckChanged(int state)
    { // RenderController::perspectiveCheckChanged()
    // reset the model's flag
    renderParameters->perspective = (state == Qt::Checked);
    // reset the interface
    renderWindow->ResetInterface();
