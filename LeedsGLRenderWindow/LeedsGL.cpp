    ...
    if (mode == POINTS) {
        ...
        for (int pri = 0; pri < primitives.size(); pri ++) {
            tasks.emplace_back([pri, &temp_res, is_in_NDCS, cutLine, &primitives, &transform](){
                ...
                auto info_0 = is_in_NDCS(tri.transformedVertices[0].position);
                auto info_1 = is_in_NDCS(tri.transformedVertices[1].position);
                auto info_2 = is_in_NDCS(tri.transformedVertices[2].position);
